...
if uploaded_file is not None:
    try:
        ...
        def generate_recommendation(row):
            ...
            if row['stockout_risk']:
                reorder_qty = max(0, row['reorder_point'] - row['current_stock'])
        ...
        col6.metric("Potential Savings from Dead Stock (₦)", f"{dead_stock_cost:,.0f}")
        ...
        with tab1:
            ...
            with col1:
                ...
                ax.axvline(90, color='orange', linestyle='--', label="Slow-Moving")
                ax.axvline(365, color='red', linestyle='--', label="Dead Stock")
                ax.legend()
                st.pyplot(fig)
            ...
            with col2:
                ...
                sizes = [
                    len(df[~ (df['slow_moving'] | df['dead_stock'] | df['stockout_risk'] | df['overstock_risk'])]),
                    df['slow_moving'].sum(),
                    df['dead_stock'].sum(),
        ...
        with tab2:
            ...
            risk_df = df[df['stockout_risk'] | df['dead_stock'] | df['overstock_risk'] | df['slow_moving']].sort_values('holding_cost_ngn', ascending=False)

            if risk_df.empty:
                st.success("All products are in healthy stock range! 🎉")
            else:
                display_df = risk_df[['product_name', 'current_stock', 'days_on_hand', 'holding_cost_ngn', 'recommendation']].copy()
                ...
                for _, row in risk_df.head(10).iterrows():
                    with st.expander(f"🔴 {row['product_name']} – Holding Cost: ₦{row['holding_cost_ngn']:,.0f}"):
                        st.write(f"**Stock:** {row['current_stock']:,} | **Days on Hand:** {row['days_on_hand']:.0f}")
        ...
        with tab3:
            ...
            col1, col2 = st.columns(2)
            with col1:
                new_lead_time = st.slider("New Lead Time (days)", 1, 90, int(df['lead_time_days'].mean()), key="lead")
            with col2:
                new_safety_days = st.slider("New Safety Stock (days)", 1, 60, int(df['safety_stock_days'].mean()), key="safety")

            sim = df.copy()
            sim['reorder_point'] = sim['avg_daily_sales'] * (new_lead_time + new_safety_days)
            sim['stockout_risk'] = sim['current_stock'] < sim['reorder_point']
            new_risk = sim['stockout_risk'].sum()
            delta = new_risk - stockout_count

            color = "save" if delta < 0 else "risk" if delta > 0 else "normal"
            st.markdown(f"<p class='big-font {color}'>Projected Stockout Risk: {new_risk} items ({'↓' if delta < 0 else '↑' if delta > 0 else '→'}{abs(delta)})</p>", unsafe_allow_html=True)
        ...
        with tab4:
            if st.button("📄 Generate Executive PDF Report"):
                ...
                story.append(Paragraph("Top Risk Items", styles['Heading2']))
                ...
                risk_data = [["Product", "Days on Hand", "Cost", "Action"]]
                for _, r in top10.iterrows():
                    action = r['recommendation'].replace("<br>", " ").replace("**", "")
                    risk_data.append([r['product_name'], f"{r['days_on_hand']:.0f}", r['holding_cost_ngn'], action[:60] + "..."])
                ...
                doc.build(story)
                buffer.seek(0)
                st.download_button("⬇️ Download PDF Report", buffer, "inventory_risk_report.pdf", "application/pdf")

        with tab5:
            st.download_button(
                "⬇️ Download Full Analysis CSV",
                df.to_csv(index=False).encode(),
                "inventory_analysis_enriched.csv",
                "text/csv"
            )
    ...
    st.info("Upload your inventory CSV to start.")
